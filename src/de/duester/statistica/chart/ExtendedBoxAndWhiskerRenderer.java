 ...
 * {@link BoxAndWhiskerCategoryDataset} and is for use with the {@link CategoryPlot}
 * class. The following enhancements have been made - Colors for the outliers and farouts
 * can be set.<br>
 * - The radius of the outliers and farouts are constant. They do not depend on the width
 * of the boxes anymore.<br>
...
public class ExtendedBoxAndWhiskerRenderer extends BoxAndWhiskerRenderer {
	...
	private boolean outliersVisible = false;
	 ...
	 * 
	 * @return The paint used to draw the outliers (never <code>null</code>).
	...
	/**
	 * Sets the paint used to color outliers and sends a {@link RendererChangeEvent} to
	 ...
	 * 
	 * @return The paint used to draw the farout values (never <code>null</code>).
	...
	/**
	 * Sets the paint used to color farout values and sends a {@link RendererChangeEvent}
	...
	/**
	 * Sets the flag that controls if outliers are drawn
	 * 
	 * @param outliersVisible
	 *            the new flag value
	 */
	...
	/**
	 * Returns the flag that controls if outliers are drawn
	 * 
	 * @return A boolean
	 */
	...
	public void drawVerticalItem(Graphics2D g2, CategoryItemRendererState state,
		...
		// Draw the box.
		Paint p = getItemOutlinePaint(row, column);
		if (p != null) {
			g2.setPaint(p);
		}
		...
		if (yQ1 != null && yQ3 != null && yMax != null && yMin != null) {
			...
			double yyMax = rangeAxis
					.valueToJava2D(yMax.doubleValue(), dataArea, location);
			double yyMin = rangeAxis
					.valueToJava2D(yMin.doubleValue(), dataArea, location);
			...
			// draw the body
			box = new Rectangle2D.Double(xx, Math.min(yyQ1, yyQ3), state.getBarWidth(),
					Math.abs(yyQ1 - yyQ3));
		...
		if (yOutliers != null) {
			...
			if (outliersVisible) {
				...
				for (Iterator<Outlier> iterator = farOutValues.iterator(); iterator
						.hasNext();) {
					Outlier outlier = (Outlier) iterator.next();
					Point2D point = outlier.getPoint();
		...
		if (state.getInfo() != null) {
			...
			if (entities != null) {
				...
				for (int i = 0; i < outliers.size(); i++) {
					...
					double outlierY = rangeAxis.java2DToValue(outlier.getY() + oRadius,
							dataArea, location);
				...
				for (int i = 0; i < farOutValues.size(); i++) {
					...
					double faroutY = rangeAxis.java2DToValue(farout.getY() + foRadius,
							dataArea, location);
					faroutTooltip = faroutTooltip.concat(Double.toString(faroutY));
					addFaroutEntity(faroutTooltip, entities, dataset, row, column,
							triangle);
				}
			}
		}
	 ...
	 *            the row index (zero-based).
	 * @param column
	 *            the column index (zero-based). TODO: Outliers and Farout values not
	 *            implemented.
	 */
	public void drawHorizontalItem(Graphics2D g2, CategoryItemRendererState state,
			Rectangle2D dataArea, CategoryPlot plot, CategoryAxis domainAxis,
			ValueAxis rangeAxis, CategoryDataset dataset, int row, int column) {
		...
		if (xQ1 != null && xQ3 != null && xMax != null && xMin != null) {
			...
			double xxMax = rangeAxis
					.valueToJava2D(xMax.doubleValue(), dataArea, location);
			double xxMin = rangeAxis
					.valueToJava2D(xMin.doubleValue(), dataArea, location);
			...
			// draw the box...
			box = new Rectangle2D.Double(Math.min(xxQ1, xxQ3), yy, Math.abs(xxQ1 - xxQ3),
					state.getBarWidth());
		...
		if (xOutliers != null) {
			for (int i = 0; i < xOutliers.size(); i++) {
				...
				if (outlier > maxOutlier.doubleValue()
					...
					Outlier faroutToAdd = new Outlier(xxFarout + (2 * foRadius), yymid,
							foRadius);
					if (dataArea.contains(faroutToAdd.getPoint())) {
						farOutValues.add(faroutToAdd);
					}
		...
		if (state.getInfo() != null) {
			...
			if (entities != null) {
				...
				for (int i = 0; i < outliers.size(); i++) {
					...
					Shape dot = createEllipse(point, oRadius);
					String outlierTooltip = "Outlier : ";
					double outlierX = rangeAxis.java2DToValue(outlier.getX() + oRadius,
							dataArea, location);
				...
				for (int i = 0; i < farOutValues.size(); i++) {
					...
					Shape triangle = createTriangleHorizontal(point, foRadius);
					String faroutTooltip = "Farout : ";
					double faroutX = rangeAxis.java2DToValue(farout.getX() - foRadius,
							dataArea, location);
	...
	private Shape createEllipse(Point2D point, double oRadius) {
		Ellipse2D dot = new Ellipse2D.Double(point.getX(), point.getY(), oRadius * 2.0,
				oRadius * 2.0);
		return dot;
	}

	/**
	 * Creates a triangle to indicate the presence of far-out values when the plot
	 ...
	 *            the hotspot.
	 */
	public void addOutlierEntity(String tooltip, EntityCollection entities,
	 ...
	 *            the hotspot.
	 */
	public void addFaroutEntity(String tooltip, EntityCollection entities,
			CategoryDataset dataset, int row, int column, Shape hotspot) {
		String url = null;
		if (getItemURLGenerator(row, column) != null) {
			url = getItemURLGenerator(row, column).generateURL(dataset, row, column);
	...
	/* (non-Javadoc)
	 * @see org.jfree.chart.renderer.category.BoxAndWhiskerRenderer#findRangeBounds(org.jfree.data.category.CategoryDataset)
	 */
	@SuppressWarnings("unchecked")
	@Override
	public Range findRangeBounds(CategoryDataset dataset) {
		Range range = super.findRangeBounds(dataset);
		if (outliersVisible) {
			DefaultBoxAndWhiskerCategoryDataset bawDataset = (DefaultBoxAndWhiskerCategoryDataset) dataset;
			for (int i = 0; i < dataset.getColumnCount(); i++) {
				List<Double> outliers = bawDataset.getOutliers(0, i);
				for (double d : outliers)
					range = Range.expandToInclude(range, d);
			}
		}
		return range;
	}
